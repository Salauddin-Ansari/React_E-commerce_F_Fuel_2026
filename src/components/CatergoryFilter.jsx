...
const CatergoryFilter = ({ selectedCategory, setSelectedCategory }) => {
  return (
    <>
      <div className="flex flex-wrap gap-3 border-gray-800 pb-6">
        <Tag className="w-5 h-5 text-orange-500 mt-2 mr-2 hidden sm:block" />
        {availableCategories.map((category) => (
          <button
            key={category}
            onClick={() => setSelectedCategory(category)}
            className={`px-5 py-2 text-sm font-bold rounded-full cursor-pointer transition duration-200 shadow-md ${
